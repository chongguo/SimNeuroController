...
class NeuralController():
    def __init__(self, model, selected_units, image_dim,w=1.,b=0.):
        # truncate model and set to evaluation mode
        self.model = model
        self.model.eval()
        self.w = torch.tensor(w,requires_grad=False).float()
        self.b = torch.tensor(b,requires_grad=False).float()
        self.dim = image_dim
    ...
    def visualize(self,niter=30,label=''):
        ...
        for i in range(niter):
            ...
            if torch.numel(self.w)==1:
                ...
                loss = -torch.mean(torch.matmul(self.w.T,x[0, self.selected_units].reshape(len(self.selected_units),-1)))-self.b
            loss.backward()
            optimizer.step()
        self.act = -loss.data.numpy()
        # Recreate image
